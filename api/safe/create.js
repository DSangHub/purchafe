...
const { requireMerchant } = require('../_merchantAuth');
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!merchant) return;
    const { ownerName, ownerAddress, purchaseDate, purchaseSource, purchaseAmount, factorySerial, notes } = req.body || {};
    ...
    await sql`INSERT INTO purchafe_safe (safe_id, verify_token, owner_name, owner_address, purchase_date, purchase_source, merchant_name, purchase_amount, factory_serial, notes, status)
      VALUES (${safeId}, ${verifyToken}, ${ownerName}, ${ownerAddress || null}, ${purchaseDate}, ${purchaseSource || null}, ${merchant.name}, ${purchaseAmount ? Number(purchaseAmount) : null}, ${factorySerial}, ${notes || null}, 'ACTIVE')`;
